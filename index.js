...
document.addEventListener('DOMContentLoaded', function () {
  const counterElements = document.querySelectorAll('.rev-main-group-stats-count');
  ...
  function startCounter(element, target) {
    ...
    function updateCounter() {
      current += increment;
      if (current < target) {
        element.textContent = Math.ceil(current).toLocaleString(); // Форматирование с разделителем тысяч
        requestAnimationFrame(updateCounter);
      } else {
        element.textContent = target.toLocaleString(); // Завершающее значение
      }
    }
...
document.addEventListener('DOMContentLoaded', function() {
  const steps = document.querySelectorAll('.recep-step');
...
document.addEventListener('DOMContentLoaded', function() {
  const imageGroups = document.querySelectorAll('.visit-before-group');

  const observer = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        entry.target.classList.add('img-visible');
        observer.unobserve(entry.target); // Отключаем наблюдение, чтобы анимация не срабатывала повторно
      }
    });
  }, {
    threshold: 0.5
  ...
  document.addEventListener("DOMContentLoaded", function () {
  const items = document.querySelectorAll(".ques-main-block-item");
  ...
  document.addEventListener("DOMContentLoaded", function () {
  const elements = document.querySelectorAll(".visit-after-line1, .visit-after-line2, .visit-after-line2-item2, .visit-after-line2-item3");
...
document.getElementById('filii-btn').addEventListener('click', function() {
  this.classList.toggle('is-active');
});

document.addEventListener('DOMContentLoaded', function () {
  const counterElements = document.querySelectorAll('.rev-main-stats-count');

  counterElements.forEach(el => {
    el.originalValue = parseInt(el.textContent.replace(/\s+/g, ''), 10); // Збереження оригінального значення
    el.textContent = '0'; // Встановлення початкового значення в нуль
  });
  ...
  function startCounter(element, target) {
    ...
    function updateCounter() {
      ...
      if (start < target) {
        ...
        element.textContent = target.toLocaleString(); // Додає пробіли для великих чисел
      }
    }
...
document.addEventListener('DOMContentLoaded', function () {
  const elements = document.querySelectorAll('.sml1-box1, .sml1-box2, .mid-box');

  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        if (entry.target.classList.contains('sml1-box1')) {
          entry.target.classList.add('fade-left', 'visible');
        } else if (entry.target.classList.contains('sml1-box2')) {
          entry.target.classList.add('fade-right', 'visible');
        } else if (entry.target.classList.contains('mid-box')) {
          entry.target.classList.add('fade-up', 'visible');
        }
      } else {
        entry.target.classList.remove('visible', 'fade-left', 'fade-right', 'fade-up');
      }
    });
  }, { threshold: 0.2 });
...
document.addEventListener('DOMContentLoaded', function () {
  const elements = document.querySelectorAll('.sml1-box1, .sml1-box2, .sml1-box4, .sml1-box5, .mid-box');

  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // В зависимости от элемента добавляем разные классы
        if (entry.target.classList.contains('sml1-box1')) {
          entry.target.classList.add('fade-left', 'visible');
        } else if (entry.target.classList.contains('sml1-box2') || entry.target.classList.contains('sml1-box4')) {
          entry.target.classList.add('fade-right', 'visible');
        } else if (entry.target.classList.contains('mid-box')) {
          entry.target.classList.add('fade-up', 'visible');
        } else if (entry.target.classList.contains('sml1-box5')) {
          entry.target.classList.add('fade-left', 'visible');
        }
      } else {
        // Удаляем классы, если элемент выходит из зоны видимости
        entry.target.classList.remove('visible', 'fade-left', 'fade-right', 'fade-up');
      }
    });
  }, { threshold: 0.2 });
...
document.addEventListener('DOMContentLoaded', function () {
  const elements = document.querySelectorAll('.sml1-box6, .sml1-box7, .sml1-box8, .sml1-box9, .box11, .box12, .mid-box');

  const observer = new IntersectionObserver((entries) => {
    entries.forEach(entry => {
      if (entry.isIntersecting) {
        // В зависимости от элемента добавляем разные классы
        if (entry.target.classList.contains('sml1-box6')) {
          entry.target.classList.add('fade-left', 'visible');
        } else if (entry.target.classList.contains('sml1-box7') || entry.target.classList.contains('sml1-box9')) {
          entry.target.classList.add('fade-right', 'visible');
        } else if (entry.target.classList.contains('mid-box')) {
          entry.target.classList.add('fade-up', 'visible');
        } else if (entry.target.classList.contains('box11') || entry.target.classList.contains('box12')) {
          entry.target.classList.add('fade-left', 'visible');
        }
      } else {
        // Удаляем классы, если элемент выходит из зоны видимости
